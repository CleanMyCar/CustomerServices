<template>
  <transition name="fade">
    <div v-if="visible" class="img-swiper lightmodal" @click.self="closeDialog">
      <div class="img-wrapper" :class="{transition: imgTransitionStatus}" :style="imgStyle">
        <!-- START: Imgs -->
        <img class="img" v-if="imgIndex === index" v-for="(img, index) in imgList"  :key="index" :src="img" alt="" draggable="false"
          @mousedown="handleMouseDown($event)" @mouseup="handleMouseUp($event)" @mousemove="handleMouseMove($event)">
        <!-- END: Imgs -->
      </div>
      <!-- START: btns -->
      <div class="btns">
        <div v-if="imgList.length !== 1" class="btn__prev" :class="{disable: imgIndex === 0}" @click="prev">
          <svg class="icon" aria-hidden="true">
            <use xlink:href="#icon-prev"></use>
          </svg>
        </div>

        <div v-if="imgList.length !== 1" class="btn__next" :class="{disable: imgIndex === imgList.length - 1}" @click="next">
          <svg class="icon" aria-hidden="true">
        ...
        <div class="toolbar-btns">
          ...
          <div class="toobar-btn toolbar-btn__rotate" @click="rotate()">
            <svg class="icon" aria-hidden="true">
...
<script>
  export default {
    ...
    watch: {
      visible(visible) {
        console.log("clicked from component ", visible);
        if (visible === true) {
          this.init()
        }
      }
